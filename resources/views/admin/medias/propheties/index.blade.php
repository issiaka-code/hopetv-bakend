...
@section('content')
                    ...
                    <div class="row g-2 d-flex flex-row justify-content-end align-items-center">
                        ...
                        <div class="col-3">
                            <input type="text" name="search" value="{{ request('search') }}" class="form-control"
                                placeholder="Rechercher une prophétie...">
                        </div>
                            ...
                            <div class="prophetie-grid-item">
                                <div class="card prophetie-card">
                                    <div class="prophetie-thumbnail-container">
                                            <div class="prophetie-thumbnail position-relative"
                                            data-prophetie-url="{{ $thumbnail_url }}"
                                            data-video-url="{{ $prophetieData->video_url ?? '' }}"
                                            data-prophetie-name="{{ $nom }}"
                                                data-media-url="{{ $media_url }}" data-media-type="{{ $media_type }}"
                                            data-has-thumbnail="{{ $prophetieData->has_thumbnail ? 'true' : 'false' }}"
                                            data-images='@json($prophetieData->images ?? [])'>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                                            ...
                                            <div class="btn-group">
                                                <button class="btn btn-sm btn-outline-info view-prophetie-btn rounded"
                                                    title="Voir la prophétie" data-prophetie-url="{{ $thumbnail_url }}"
                                                ...
                                                <button
                                                    class="btn btn-sm btn-outline-primary edit-prophetie-btn mx-1 rounded"
                                                    title="Modifier la prophétie"
                                                    data-prophetie-id="{{ $id }}">
                                                    <i class="fas fa-edit"></i>
                                                </button>

                                                <form action="{{ route('propheties.destroy', $id) }}" method="POST"
                                                    class="d-inline delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-outline-danger rounded"
                                                        title="Supprimer la prophétie"
                                                        onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette prophétie ?')">
    ...
    <!-- Modals -->
    @include('admin.medias.propheties.modals.add')
    @include('admin.medias.propheties.modals.edit')
    @include('admin.medias.propheties.modals.view')
@endsection
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('input[name="media_type"]', '#addprophetieForm').change(function() {
            ...
            $('#addAudioFile, #addVideoFile, #addPdfFile, #addImageFiles, #addImageCoverFile, #addAudioImageFile, #addVideoImageFile, #addPdfImageFile')
                .on('change', function() {
                    ...
                    // DÃ©terminer le libellÃ© par dÃ©faut selon l'input
                    const id = this.id;
            ...
            $('#addprophetieModal').on('hidden.bs.modal', function() {
                $('#addprophetieForm')[0].reset();
            ...
            $('input[name="media_type"]', '#editprophetieForm').change(function() {
                const selectedType = $(this).val();
            ...
            $(document).on('click', '.edit-prophetie-btn', function() {
                ...
                $.ajax({
                    url: "{{ route('propheties.edit', ':id') }}".replace(':id', prophetieId),
                    method: 'GET',
                    success: function(data) {
                        $('#editprophetieNom').val(data.nom);
                        $('#editprophetieDescription').val(data.description);
                        $('#editprophetieForm').attr('action',
                            "{{ route('propheties.update', ':id') }}".replace(':id',
                                prophetieId));
            ...
            $(document).on('click', '.view-prophetie-btn, .prophetie-thumbnail', function() {
                ...
                const prophetieDescription = $(this).closest('.prophetie-card').find('.card-text').attr(
                    'title') || '';
                ...
                if (mediaType === 'audio') {
                    ...
                    $('#modalIframePlayer').attr('src', prophetieUrl);
    ...
    <img class="d-block w-100" src="' + url + '" alt="Image ' + (idx + 1) + '" style="max-height: 450px; object-fit: contain; background: #000;"/>\
                ...
                $('#prophetieTitle').text(prophetieName);
                $('#prophetieDescription').text(prophetieDescription);
                $('#prophetieViewModal').modal('show');
            });

            // ===== NETTOYAGE DU MODAL =====
            $('#prophetieViewModal').on('hidden.bs.modal', function() {
                // ArrÃªter tous les mÃ©dias
                $('#modalAudioPlayer').get(0).pause();
                $('#modalVideoPlayer').get(0).pause();

                // RÃ©initialiser les sources
                ...
                $('#prophetieTitle, #prophetieDescription, #mediaTypeBadge').text('');
            });

            // ===== LECTURE AUTOMATIQUE =====
            $('#prophetieViewModal').on('shown.bs.modal', function() {
